...
const renderRules = {
  ...
  image: (node, children, parent, styles) => {
    const source = node.attributes.src.startsWith("/") ? 'file://' + node.attributes.src : node.attributes.src;
    return <FitImage indicator={true} indicatorColor={"#f0ad4e"} indicatorSize={"large"}
                     key={node.key} style={styles.image} source={{ uri: source }} />;
  },
};
